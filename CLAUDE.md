# metalsmith-blog-lists - Development Context
...
Builds four blog collections from post frontmatter and writes them into
`metalsmith.metadata()` for templates to consume: `allSortedBlogPosts`,
`latestBlogPosts`, `featuredBlogPosts`, and `annualizedBlogPosts`. It reads
frontmatter only and never touches file contents. Logic is split across
single-purpose helpers in `src/utils/` (config, blogProperty, sorting,
collections).
...
Node 22's stable `require(esm)` support. It has no runtime dependencies.
...
`Metalsmith` instance so `metalsmith.debug()` and `metalsmith.metadata()` are
...
Specific things to grep for: option names and defaults (`latestQuantity`,
`featuredQuantity`, `featuredPostOrder`, `fileExtension`, `blogDirectory`,
`blogObject`, `usePermalinks`) that no longer match `src/`, code examples that
import removed exports, "What's New" callouts referencing past releases,
broken links to renamed sections.
...
/
├── src/
│   ├── index.js              # Plugin entry — collect posts, derive lists
│   └── utils/                # config, blogProperty, sorting, collections
├── test/
│   ├── index.test.js         # node:test against src/ directly
│   └── fixtures/             # Sample sites per scenario
├── docs/
